...
1. 使用范围：CMS是老年代垃圾收集器，需要配合新生代垃圾收集器（Serial or ParNew）一起使用。G1收集器收集范围是老年代和新生代，不需要结合其他收集器

2. stop the world的时间：CMS是一种以获得最短回收时间为目标的手机其。G1收集器可以预测垃圾回收的停顿时间。

3. 垃圾碎片：CMS采用标记-清楚算法，会产生大量的碎片空间。G1采用的是标记整理的算法，进行了空间整合，降低了内存空间的碎片

4. 回收过程不同

5. 对内存区域划分的不同，G1划分为多个大小相等的独立区域
...
1. CMS收集器对CPU资源非常敏感。并发执行阶段会占用CPU资源

2. CMS收集器无法处理浮动垃圾

3. 采用标记-清除算法实现的收集器，收集结束时会产生大量的空间碎片，分配大对象时，由于碎片空间过多，没有完整的空间提供给大对象，会触发full gc。
